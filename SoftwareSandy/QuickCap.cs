            ...
            if (dsConfig.Tables.Count > 0 && dsConfig.Tables[0].Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dsConfig.Tables[0].Rows)
                {
                    ...
                    if (dr["MSSQLDBNAME"] != null && dr["MSSQLDBNAME"] != DBNull.Value && !string.IsNullOrEmpty(Convert.ToString(dr["MSSQLDBNAME"])))
                    {
                        ...
                        strMSSQLConString = String.Format("server=172.16.0.37\\quickcap;database={0};user Id=" + dsConfig.Tables[0].Rows[rowID]["MSSQLDBUSERNAME"].ToString() + ";password=" + dsConfig.Tables[0].Rows[rowID]["MSSQLDBPASSWORD"].ToString(), strMSSQLDB);
